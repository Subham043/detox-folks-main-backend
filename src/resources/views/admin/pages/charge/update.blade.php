...
<div class="page-content">
    <div class="container-fluid">
        ...
        <!-- start page title -->
        @can('list charges')
        @include('admin.includes.breadcrumb', ['page'=>'Charge', 'page_link'=>route('charge.paginate.get'), 'list'=>['Update']])
        @endcan
        <!-- end page title -->
        ...
        <div class="row" id="image-container">
            ...
            <div class="col-lg-12">
                <form id="countryForm" method="post" action="{{route('charge.update.post', $data->id)}}" enctype="multipart/form-data">
                @csrf
                    <div class="card">
                            ...
                            <div class="live-preview">
                                <div class="row gy-4">
                                    <div class="col-xxl-6 col-md-6">
                                        @include('admin.includes.input', ['key'=>'charges_name', 'label'=>'Charges Name', 'value'=>$data->charges_name])
                                    </div>
                                    <div class="col-xxl-6 col-md-6">
                                        @include('admin.includes.input', ['key'=>'charges_slug', 'label'=>'Charges Slug', 'value'=>$data->charges_slug])
                                    </div>
                                    <div class="col-xxl-6 col-md-6">
                                        @include('admin.includes.input', ['key'=>'charges_in_amount', 'label'=>'Charges In Amount', 'value'=>$data->charges_in_amount])
                                    </div>
                                    <div class="col-xxl-6 col-md-6">
                                        @include('admin.includes.input', ['key'=>'include_charges_for_cart_price_below', 'label'=>'Include Charges For Cart Price Below', 'value'=>$data->include_charges_for_cart_price_below])
                                    </div>
...
.addField('#charges_name', [
  ...
  .onSuccess(async (event) => {
    ...
    try {
        ...
        const response = await axios.post('{{route('charge.update.post', $data->id)}}', formData)
