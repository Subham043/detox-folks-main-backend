...
class AssignedOrderForAgentPaginateController extends Controller
{
    ...
    public function payUMoneyView(string $order_id)
    {
        ...
        return redirect(route('delivery_management.agent.order_detail.get', $order_id))->with('error_status', "QR Code generation failed");
    ...
    public function payUMoneyView2()
    {
        ...
        return redirect(route('delivery_management.agent.order_detail.get', "abe6160"))->with('error_status', "QR Code generation failed");
    ...
    public function verifyPayUQRPayment2()
    {
        ...
        if($resp['msg']=="Transaction is Successful"){
            $rData = (new PayUService)->get_order((string) "abe6160");
            if($rData->status == 1){
                DB::insert('insert into test_upi (data) values (?)', [json_encode($rData['transaction_details']['abe6160'])]);
            }
            return response()->json([
                'data' => true
            ], 200);
